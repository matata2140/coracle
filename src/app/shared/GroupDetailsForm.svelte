<script lang="ts">
  ...
  import type {GroupMeta, Person} from "src/engine"
  import {searchRelays, normalizeRelayUrl} from "src/engine"

  export let onSubmit
  export let mode = "create"
  export let showMembers = false
  export let meta: GroupMeta = {}
  export let relays: string[] = []
  export let members: Person[] = []
  export let published_at: number = null

  const isAlreadyPublic = published_at
  ...
  const submit = async () => {
    ...
    await onSubmit({meta, relays, members, published_at})
...
<form on:submit|preventDefault={submit} in:fly={{y: 20}}>
  <Content>
    ...
    <div class="flex w-full flex-col gap-8">
      ...
      <Field label="Banner">
        <ImageInput bind:value={meta.banner} icon="image" maxWidth={2048} maxHeight={480} />
        <div slot="info">A banner for the group's page</div>
      </Field>
      <Field label="About">
        <Textarea bind:value={meta.about} />
        <div slot="info">The group's decription</div>
      </Field>
      <Field label="Relays">
        <MultiSelect search={searchRelayUrls} bind:value={relays} termToItem={normalizeRelayUrl}>
          <i slot="before" class="fa fa-clipboard" />
        </MultiSelect>
        <div slot="info">
      ...
      <FieldInline label="Make Public">
        <Toggle disabled={!!isAlreadyPublic} bind:value={published_at} />
        <div slot="info">
